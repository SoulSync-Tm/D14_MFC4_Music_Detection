...
def list_songs():
    ...
    return {"count": len(songs), "songs": [{"id": sid, "name": name} for sid, name in songs]}


@router.post("/match")
async def match_song(
    file: UploadFile = File(...),
    phone_mode: bool = Query(False, description="Apply 300–3400 Hz bandpass (phone/mic simulation)"),
    ...
    return {
        ...
        "results": [
            {"song_id": m.song_id, "song_name": m.song_name, "confidence": round(m.confidence, 4)}
